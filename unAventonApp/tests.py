...
class MockModels():
    ...
    def mock_viaje(self, tipo, cbu, auto):
        return Viaje.objects.create_viaje(
            ...
            destino='BsAs',
            duracion=2,
            fecha_hora_salida=datetime.datetime(2099, 11, 20, 20, 8, 7, 127325, tzinfo=pytz.UTC),
            cuenta_bancaria=cbu,
            se_repite=(1,[]),
            auto=auto,
            gasto_total=500,
...
class  BaseTest(TestCase):
    ...
    def createSomeData(self):
        ...
        self.copiloto_1.set_calificar_piloto(1, self.conductor_1, self.viaje1, 'todo bien')
        self.conductor_1.set_calificar_copiloto(1, self.copiloto_1, self.viaje1, 'genial')
...
class TestMetodosDeModels(BaseTest):
    ...
    def test_copiloto_confirmado_tiene_calificaciones_pendientes_para_el_piloto(self):
        self.assertEqual(len(self.copiloto_2.get_calificaciones_pendientes_para_piloto()), 2,
    ...
    def test_copiloto_no_confirmado_no_tiene_calificaciones_pendientes_para_el_piloto(self):
        self.assertEqual(len(self.copiloto_4.get_calificaciones_pendientes_para_piloto()), 0)

    def test_copiloto_sin_viajes_no_tiene_calificaciones_pendientes_para_el_piloto(self):
        self.assertEqual(len(self.copiloto_6.get_calificaciones_pendientes_para_piloto()), 0, msg='No deberia, si no tiene viajes!!')

    def test_piloto_tiene_calificaciones_pendientes_para_copilotos_de_su_viaje(self):
        self.assertEqual(len(self.conductor_1.get_calificaciones_pendientes_para_copilotos()), 2, msg='error, aun debe calificar a copiloto_2 y copiloto_3')

    def test_viajes_en_espera_como_copiloto(self):

        self.assertEqual(self.copiloto_1.get_viajes_en_espera_como_copiloto().count() == 1, True)
    ...
    def test_viajes_confirmados_como_copiloto(self):
        ...
        '''
        self.assertEqual(self.copiloto_1.get_viajes_confirmados_como_copiloto().count() == 1, True)
    ...
    def test_un_usuario_es_copiloto_en_un_viaje_dado(self):
        ...
        '''
        self.assertEqual(self.copiloto_2.es_copiloto_en_viaje(self.viaje1), True)

    def test_un_usuario_no_es_copiloto_en_un_viaje_dado(self):
        self.assertEqual(self.copiloto_6.es_copiloto_en_viaje(self.viaje1), False, msg='no deberia ser copiloto de ese viaje')
    ...
    def test_un_usuario_es_piloto_en_un_viaje_dado(self):
        ...
        mocker.mock_ViajeCopiloto(self.copiloto_2, viaje2)
        '''
        self.assertEqual(self.conductor_1.es_piloto_en_viaje(self.viaje1), True, 'No es conductor de ese viaje')

    def test_un_usuario_no_es_piloto_en_un_viaje_dado(self):
        self.assertEqual(self.conductor_2.es_piloto_en_viaje(self.viaje1), False, 'No es conductor de ese viaje')
    ...
    def test_conductor_califica_copiloto_de_su_viaje(self):
        ...
        '''
        self.assertEqual(self.copiloto_1.get_calificacion_como_copiloto(), 1, msg='El copiloto no pertenece a su viaje')
    
    def test_conductor_califica_copiloto_de_un_viaje_que_no_es_suyo(self):
        self.conductor_1.set_calificar_copiloto(1, self.copiloto_5, self.viaje2, 'se re cagaba')
        self.assertEqual(self.copiloto_5.get_calificacion_como_copiloto(), None, msg='No deberia tener calificaiones porque lo califico un conductor de un viaje distinto')
    
    def test_conductor_calificado_como_piloto_con_calificacion_previa(self):

        self.assertEqual(self.conductor_1.get_calificacion_como_piloto(), 1, msg='Lo califico solo 1 usuario, debe ser 1')

    def test_conductor_sin_calificar_como_piloto(self):

        self.assertEqual(self.conductor_2.get_calificacion_como_piloto(), None, msg='Deberia ser None, nadie lo califico como piloto aun')

    def test_pasajero_calificado_como_copiloto(self):
    
        self.assertEqual(self.copiloto_1.get_calificacion_como_copiloto(), 1, msg='Deberia ser 1, solo lo califiacron una vez')

    def test_pasajero_sin_calificar_como_copiloto(self):
        self.assertEqual(self.copiloto_4.get_calificacion_como_copiloto(), None, msg='Deberia ser None, nadie lo califico')

    def test_cantidad_de_tarjetas_de_un_usuario(self):
        self.assertEqual(len(self.conductor_1.get_tarjetas_de_credito()) == 1, True, msg='deberia tener solo 1 tarjeta asociada')
